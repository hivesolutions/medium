...
<div class="fields">
    ${foreach item=field_value key=field_key from=fields_map}
        <div id="field-${out value=field_key xml_escape=True /}">${out value=field_value xml_escape=True /}</div>
    ${/foreach}
</div>
    ...
    <div id="main-wrapper">
        ...
        <div id="right">
            <div id="commits">
                ...
                <span id="commits-value" class="stat">${out value=fields_map.commits xml_escape=True /}</span>
            </div>
            <div id="bugs">
                <p class="header purple">bugs <span class="sub-header">/24h</span></p>
                <span id="bugs-value" class="stat">${out value=fields_map.bugs xml_escape=True /}</span>
            </div>
            <div id="elcommitator" class="clear">
                <p class="header"><span class="header blue">commiter of the day</span></p>
                <span id="commiter-value" class="text-header">${out value=fields_map.commiter xml_escape=True /}</span><span class="sub-header-commit">/<span id="commiter_count-value">${out value=fields_map.commiter_count xml_escape=True /}</span> commits</span>
            </div>
        </div>
    </div>
    <div class="clear"></div>
            ...
            ${foreach item=ticker_message from=ticker_messages}
                ${if item=ticker_message.type value="error" operator="eq"}
                    <div class="icon-ticker icon-ticker-error">
                        <span class="text">${out value=ticker_message.value xml_escape=True /}</span>
                        <span class="sub-header-ticker orange">${out value=ticker_message.sub_value xml_escape=True /}</span>
                    </div>
                ${elif item=ticker_message.type value="warning" operator="eq" /}
                    <div class="icon-ticker icon-ticker-warning">
                        <span class="text">${out value=ticker_message.value xml_escape=True /}</span>
                        <span class="sub-header-ticker yellow">${out value=ticker_message.sub_value xml_escape=True /}</span>
                    </div>
                ${elif item=ticker_message.type value="information" operator="eq" /}
                    <div class="icon-ticker icon-ticker-information">
                        <span class="text">${out value=ticker_message.value xml_escape=True /}</span>
                        <span class="sub-header-ticker blue">${out value=ticker_message.sub_value xml_escape=True /}</span>
                    </div>
                ${/if}
            ${/foreach}
            <div class="clear"></div>
